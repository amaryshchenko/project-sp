...
![Network ping sweep reveals 7 live hosts across the subnet](service_enumeration_all_ports.png)
...
![DC (10.0.0.5): DNS/53, HTTP/80 (IIS 10.0), Kerberos/88, LDAP/389, SMB/445, RDP/3389 — classic Active Directory fingerprint](se_5.png)

```bash
nmap -sV -sC 10.0.0.8    # Corporate Server
```

![Corp Server (10.0.0.8): SSH/22 running OpenSSH 8.9p1 Ubuntu — primary attack entry point identified](se_8.png)

```bash
nmap -sV -sC 10.0.0.100  # Windows Workstation
```

![Windows Workstation (10.0.0.100): msrpc/135, WinRM/5985 — lateral movement vector identified](se_100.png)

```bash
nmap -sV -sC 10.0.0.101  # Linux Client (Jane)
```

![Linux Client (10.0.0.101): SSH/22 running OpenSSH 8.9p1 Ubuntu — Jane's machine discovered](se_101.png)
...
![Hydra successfully cracks root@10.0.0.8 — password: november — found in rockyou.txt wordlist](hydra_ssh_8.png)
...
![Hydra confirms root / november also valid on 10.0.0.101 — same weak password reused across both systems](hydra_ssh_101.png)
...
![Attacker's fake "Verify Your Password" credential-harvesting page hosted at http://10.0.0.9 — identical look to a legitimate corporate portal](fake_verify_page_example.png)
...
![Phishing email captured in Jane's MailHog terminal parser — from: project-sp-hrteam@corp.project-sp-dc.com — HTML body contains malicious "Verify My Account" link pointing to 10.0.0.9](phishing_email_jane_parser.png)

![Jane's MailHog inbox (localhost:8025) shows the phishing email rendered — "Update Password!" with clickable "Verify My Account" hyperlink](phishing_email_web.png)

### Credential Capture

Jane clicks the link and enters her real credentials into the fake page:

![Jane enters her credentials into the phishing site — username: jane, password: @password123! — page is indistinguishable from a real portal](fake_verify_real_example.png)
...
![creds.log confirms two captured credential sets — amaryshchenko:hello123 (12:37) and jane:@password123! (12:41)](success_creds_fake_verify.png)
...
![SSH login as jane@10.0.0.101 successful — Welcome to Ubuntu 22.04 LTS (linux-client) — legitimate foothold established on the internal network](ssh_jane_result.png)
...
![NetExec confirms Administrator:@Deeboodah1! valid on WIN11SP (10.0.0.100) — (Pwn3d!) tag indicates full administrative WinRM access](netexec_enumeration.png)
...
![Evil-WinRM shell established on 10.0.0.100 — whoami confirms corp\administrator — full interactive PowerShell session active](evil-winrm_post-exploitation.png)
...
![xfreerdp command launched from Kali — connecting to DC (10.0.0.5) with Administrator credentials](xfreerdp.png)

![Full GUI RDP session on Domain Controller (FreeRDP: 10.0.0.5) — Windows Server 2025 Server Manager visible — Keys to the Kingdom obtained](xfreerdp_result.png)
...
![SCP transfer from DC PowerShell — secrets.txt sent to kali@10.0.0.9 — 100% transfer confirmed at 2.6KB/s](scp_exfiltration_secrets.png)

![Attacker verifies file received on Kali — cat secrets.txt returns the stolen contents](scp_exfiltration_result.png)

![Contents of secrets.txt: $PROJECT-SP{123HELLO} — sensitive production credential/flag data successfully exfiltrated](stolen_data_example.png)
...
![Rogue account project-sp-user created via Administrator PowerShell on DC — added to local Administrators and Domain Admins groups successfully](eviluser_creation.png)

```powershell
net user project-sp-user /domain
```

![net user /domain confirms project-sp-user: Account active, Local Group = *Administrators, Global Group = *Domain Admins + *Domain Users](confirm_eviluser.png)
...
![Python HTTP server running on Kali port 8000 — serving reverse.ps1 — simultaneously showing confirmed eviluser creation in the background](pythonserv.png)

The Domain Controller downloads the file by browsing to `http://10.0.0.9:8000` via the RDP session browser:

![Browser on DC accesses http://10.0.0.9:8000 — directory listing shows reverse.ps1 available for download](download_reverse_pythonserv.png)

The file is saved to the persistence location:

![File Explorer on DC confirms reverse.ps1 placed in AppData\Local\Microsoft\Windows\ — hidden among legitimate Windows system files](reverse_file_location.png)
...
![schtasks command creates PersistenceTask — SUCCESS: The scheduled task has been successfully created — runs daily at 12:00 PM](scheduled_task_reverse_shell_1.png)

![Task Scheduler GUI confirms PersistenceTask — Status: Ready — Trigger: At 12:00 PM every day — Author: CORP\Administrator](scheduled_task_reverse_shell_2.png)
...
![PowerShell on DC executes reverse.ps1 — initial execution policy block shown, then bypassed — script runs with [R] Run once selection](scheduled_task_reverse_shell_3.png)

![Netcat on Kali catches the reverse shell callback from 10.0.0.5 — "Connected to reverse shell!" — whoami confirms corp\administrator](persistence_result.png)
